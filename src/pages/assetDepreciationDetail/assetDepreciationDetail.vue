<template>
    <div class="layer-voucher-detail">
      <div class="form-voucher-detail">
        <div class="form-header-voucher-detail">
          <div class="form-title-voucher-detail">Tính hao mòn</div>
          <div class="blank"></div>
          ...
          <div class="voucher-info">
            <div class="voucher-info-row-1">
              ...
              <div class="form-input-voucher-detail">
                ...
                <MDatetime
                  v-model="voucher.voucher.posted_date"
                  :key="componentKey"
                  :bottom="true"
                  v-outside="
                    () => {
  ...
  export default {
    ...
    methods: {
      ...
      changeDataCombo(dataEmit, key, index, data) {
        if (key == "Asset") {
          ...
          data[index].description = 'Điều chuyển tài sản ' + dataEmit.asset_name;     
        }
        if(key == "FromDepartment") {     
          data[index].from_department_id = dataEmit.department_id;
        }
        if(key == "ToDepartment") {     
          data[index].to_department_id = dataEmit.department_id;
        }
      ...
      editVoucher() {
        ...
        var result = {
          voucher: {
            ...
            amount: this.voucher.voucher.price,
            posted_date: this.voucher.voucher.voucher_date,
          },
          ...
          .then((res) => {
            this.voucher = {
              ...
              voucher: {
                ...
                price: res.data.voucher.amount,
              },
            };
            this.$emit("dataForCountAvailable", res.data.voucherDetails);
            this.totalValue(this.dataForTable);
            this.$emit("cancelLoading");
            this.forceRerender();
          })
      ...
      totalValue(listAsset) {
        var totalCost = 0;
        listAsset.forEach((item) => (totalCost += item.amount || 0));   
        var totalWastageRate = 0;
        listAsset.forEach((item) => (totalWastageRate += item.wastage_rate || 0));  
        var totalYearDepreciationValue = 0;
        listAsset.forEach((item) => (totalYearDepreciationValue += item.year_depreciation_value || 0)); 
        var totalDepreciationValue = 0;
        listAsset.forEach((item) => (totalDepreciationValue += item.depreciation_value || 0)); 
        var totalPrice = 0;
        listAsset.forEach((item) => (totalPrice += item.price || 0)); 
        this.arrayTotal = [totalCost, totalWastageRate, totalYearDepreciationValue, totalDepreciationValue, totalPrice];
    ...
    data() {
      return {
        ...
        voucherAPI: config.assetDepreciationAPI,
        voucherTh: resource.voucherTh,
        voucherDetailThForm: resource.assetDepreciationDetailTH,
  ...
  <style scoped>
  @import url(./assetDepreciationDetail.css);
  </style>
